...
contract("Cartoon Characters", async (accounts) => {
  ...
  describe("Deployment", async () => {
    it("contract has an address", async () => {
      const address = await cartoonCharacters.address;
    ...
    it("has a name", async () => {
      const name = await cartoonCharacters.collectionName();
      assert.equal(name, "Crypto Boys Collection");
    });

    it("has a symbol", async () => {
      const symbol = await cartoonCharacters.collectionNameSymbol();
  ...
  describe("application features", async () => {
    it("allows users to mint ERC721 token", async () => {
      ...
      let tokenURIExists;
      tokenURIExists = await cartoonCharacters.tokenURIExists(
      ...
      let tokenNameExists;
      tokenNameExists = await cartoonCharacters.tokenNameExists("myCBNFT", {
        from: accounts[0],
      });
      ...
      for (let i = 0; i < colorsArray1.length; i++) {
        colorExists = await cartoonCharacters.colorExists(colorsArray1[i], {
          from: accounts[0],
        });
        assert.equal(colorExists, false);
      }

      result = await cartoonCharacters.mintCartoonCharacter(
        "myCBNFT",
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2",
      ...
      let cartoon;
      cartoon = await cartoonCharacters.allCartoonCharacter(1, {
        from: accounts[0],
      });
      assert.equal(cartoon.tokenId.toNumber(), 1);
      assert.equal(cartoon.tokenName, "myCBNFT");
      assert.equal(
        cartoon.tokenURI,
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2"
      );
      assert.equal(cartoon.mintedBy, accounts[0]);
      assert.equal(cartoon.currentOwner, accounts[0]);
      assert.equal(
        cartoon.previousOwner,
        0x0000000000000000000000000000000000000000
      );
      assert.equal(web3.utils.fromWei(cartoon.price, "ether"), 1);
      assert.equal(cartoon.numberOfTransfers.toNumber(), 0);
      assert.equal(cartoon.forSale, true);
      ...
      await cartoonCharacters.mintCartoonCharacter(
        "myCBNFT2",
    ...
    it("returns address of the token's owner", async () => {
      const tokenOwner = await cartoonCharacters.getTokenOwner(2);
      assert.equal(tokenOwner, accounts[1]);
    });

    // returns tokenURI of the token
    it("returns metadata of a token", async () => {
      const tokenMetaData = await cartoonCharacters.getTokenMetaData(2);
      assert.equal(
        tokenMetaData,
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPQRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2"
      );
    ...
    it("returns total number of tokens minted so far", async () => {
      const totalNumberOfTokensMinted = await cartoonCharacters.getNumberOfTokensMinted();
      assert.equal(totalNumberOfTokensMinted.toNumber(), 4);
    });

    it("returns total number of tokens owned by an address", async () => {
      const totalNumberOfTokensOwnedByAnAddress = await cartoonCharacters.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
    ...
    it("allows users to buy token for specified ethers", async () => {
      const oldTokenOwner = await cartoonCharacters.getTokenOwner(1);
      ...
      oldTotalNumberOfTokensOwnedBySeller = await cartoonCharacters.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
      ...
      const newTokenOwner = await cartoonCharacters.getTokenOwner(1);
      ...
      newTotalNumberOfTokensOwnedBySeller = await cartoonCharacters.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
      ...
      await cartoonCharacters.buyToken(56, {
        from: accounts[4],
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;

      await cartoonCharacters.buyToken(3, {
        from: accounts[0],
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;
    });
    ...
    it("allows users to change token price", async () => {
      ...
      await cartoonCharacters.changeTokenPrice(82, web3.utils.toWei("3", "Ether"), {
        from: accounts[2],
      }).should.be.rejected;
    ...
    it("allows users to toggle between setting the token for sale or not for sale", async () => {
      ...
      result = await cartoonCharacters.toggleForSale(1, { from: accounts[2] });

      cartoon = await cartoonCharacters.allCartoonCharacter(1, {
        from: accounts[0],
      });
      assert.equal(cartoon.forSale, false);
      ...
      await cartoonCharacters.toggleForSale(1, {
        from: 0x0000000000000000000000000000000000000000,
      }).should.be.rejected;

      await cartoonCharacters.toggleForSale(94, { from: accounts[2] }).should.be
        .rejected;

      await cartoonCharacters.toggleForSale(1, { from: accounts[8] }).should.be
        .rejected;
    });
  });
});